...
const AllCakes = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await axios.get(
          `https://www.ssdipl.com/api/get-product-by-subcatname/${subcatname}`
        );
    ...
    const fetchProductById = async (subCategoryId) => {
      try {
        const res = await axios.get(
          `https://www.ssdipl.com/api/get-product-by-subCategoryId/${subCategoryId}`
        );
  ...
  const imageUrl =
    subcategoryInfo?.image
      ? subcategoryInfo.image.startsWith("http")
        ? subcategoryInfo.image
        : `https://www.ssdipl.com/${subcategoryInfo.image}`
      : Banner1;
      ...
      {subcategoryInfo?.banner && <section className="cakeBannerSlider">
        ...
        <div >
          <div className="bannerBox">
            <img src={`https://www.ssdipl.com/${subcategoryInfo?.banner}`} alt="Cake Banner" />
          </div>
        </div>
...
//     try {
//       let url = `https://www.ssdipl.com/api/get-product-by-subcatname/${subcatname}`;
...
//                         <img
//                           src={`https://www.ssdipl.com/${item.productImage[0]}`}
//                           className="w-100"
//                           alt="images"
//                         />
