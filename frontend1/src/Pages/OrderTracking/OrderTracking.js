...
//     try {
//       const res = await axios.get(`htttp://localhost:7000/api/checkout/user/${user}`)
...
//               <h4>Order Details</h4>
//               <span className="status-badge">{order?.orderStatus || 'Out for Delivery'}</span>
//             </div>

//             <p className="text-muted idSec">
//               Order ID <b>#CK{order?._id || Date.now().toString().slice(-6)}</b>
//             </p>
    ...
    const fetchOrderByUser = async () => {
      try {
        const res = await axios.get(
          `htttp://localhost:7000/api/checkout/user/${user}`
        );
        setOrderData(res.data.data || []);
      } catch (e) {
        console.log(e);
                ...
                {items.map((item, index) => (
                  <div className="col-md-6" key={index}>
                    <div className="item-card">
                      <div className="item-img">
                        <img
                          src={`htttp://localhost:7000/${item.image.replace(
                            /\\/g,
                            "/"
                          )}`}
                          alt={item.name}
