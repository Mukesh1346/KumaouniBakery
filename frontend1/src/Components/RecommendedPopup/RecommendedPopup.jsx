...
const RecommendedPopup = ({ open, onClose, productId }) => {
    ...
    const fetchCategories = async () => {
      try {
        const res = await axios.get(
          "https://www.ssdipl.com/api/recommended-category/get-recommended-category"
        );
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const res = await axios.get(
          `https://www.ssdipl.com/api/recommended-product/get-product-by-category/${activeCategory}`
        );
        ...
        <div className="rp-grid">
          {product.map((p) => (
            <div className="rp-card" key={p._id}>
              <img src={`https://www.ssdipl.com/${p?.productImage[0]}`} alt={p?.productName} />
              <h6>{p.productName}</h6>
              <p>₹ {p.price}</p>
