...
//     try {
//       const res = await axios.get(`https://www.ssdipl.com/api/wishlist/get-wishlist/${user}`);
//       if (res.status === 200) {
...
//                 <img src={`https://www.ssdipl.com/${item?.productId?.productImage[0] || item?.productId?.productImage}`} alt={item?.name} />
...
const Wishlist = () => {
  ...
  const fetchWishlist = async () => {
    try {
      const res = await axios.get(
        `https://www.ssdipl.com/api/wishlist/get-wishlist/${user}`
      );
      if (res.status === 200) {
        setWishlist(res.data.data);
      }
  ...
  const handleRemove = async (productId) => {
    try {
      await axios.delete(
        "https://www.ssdipl.com/api/wishlist/remove-wishlist",
        {
          data: { user, productId },
        }
      );
