    ...
    const fetchProductTags = async () => {
      try {
        const response = await axios.get(
          "https://bakery-46ac.onrender.com/api/get-producttag"
  ...
  const deleteProductTag = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(
            `https://bakery-46ac.onrender.com/api/delete-producttag/${id}`
          ); // Replace with your actual API endpoint
            ...
            {productTags.length > 0 ? (
              productTags.map((tag) => (
                <tr key={tag._id}>
                  ...
                  <td>
                    <img
                      src={`https://bakery-46ac.onrender.com/${tag.image}`}
                      alt={tag.tagHeading}
                  ...
                  <td>
                    {tag.priceRange.map((range, index) => (
                      <div key={index}>
                        ...
                        <img
                          src={`https://bakery-46ac.onrender.com/${range.priceRangeImage}`}
